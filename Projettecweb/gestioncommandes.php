...
<div class="header">
    <form action="addcommande.php" method='POST'>
        <input type='submit' value='Ajouter une commande' class='ajout'>
    </form>
    <form action="searchcommande.php" method='POST'>
...
require_once($_SERVER['DOCUMENT_ROOT']."/Projettecweb/include/dbaccess.php");
    $db = new Dbaccess();
    $db->query("select count(commande_num) as c from commande");
    $count = $db->resultSet2();
    ...
    $dba = new Dbaccess();
    $dba->query("select * from commande limit $debut,2");
