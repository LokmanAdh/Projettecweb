...
<div class="container">
<div class="header">
    <form action="addapprovisionnement.php" method='POST'>
        <input type='submit' value='Ajouter une approvisionnement' class='ajout'>
    </form>
    <form action="searchapprovisionnements.php" method='POST'>
...
require_once($_SERVER['DOCUMENT_ROOT']."/Projettecweb/include/dbaccess.php");
    $db = new Dbaccess();
    $db->query("select count(A_num) as a from approvisionnement");
    $count = $db->resultSet2();
    ...
    $dba = new Dbaccess();
    $dba->query("select * from approvisionnement limit $debut,2");
